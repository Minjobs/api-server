...
export const verifySlip = async (req, res) => {
    ...
    try {
        ...
        // 3. 보안 검증 (Anti-Fraud)
        // A. 영수증 중복 사용 확인 (transRef가 이미 DB에 있는지)
        ...
        // 4. 모든 검증 통과 -> 코인 지급 및 주문 완료 처리 (트랜잭션 시작)
        const connection = await db.getConnection();
        await connection.beginTransaction();

        try {
            // [수정된 승인 로직: Atomic Update]
            // 1. 주문 상태를 success로 바꾸되, 현재 상태가 반드시 pending이어야 함 (중요!)
            const [result] = await connection.execute(
                `UPDATE payment_transactions 
                 SET status = 'success', trans_ref = ? 
                 WHERE id = ? AND status = 'pending'`, // 'pending'인 경우만 업데이트 허용
                [transRef, transactionId]
            );

            // result.affectedRows가 0이면, 이미 다른 탭이나 요청에서 처리가 끝났다는 뜻입니다.
            if (result.affectedRows === 0) {
                await connection.rollback();
                return res.status(400).json({ error: '이미 처리된 주문이거나 유효하지 않은 요청입니다.' });
            }

            // 2. 위에서 상태 변경이 성공(1행 변경)했을 때만 유저에게 코인을 지급합니다.
            await connection.execute(
                `UPDATE users SET coins = coins + ? WHERE line_user_id = ?`,
                [order.coin_amount, order.line_user_id]
            );

            // 3. 모든 작업이 성공하면 확정(Commit)
